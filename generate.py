...
def gh(url):
    hdr = {"Accept": "application/vnd.github+json", "User-Agent": "harbourmasters-ports-source"}
    if TOKEN:
...
def ipa_version(url, fallback):
    ...
    try:
        with urllib.request.urlopen(urllib.request.Request(url, headers={"User-Agent": "harbourmasters-ports-source"})) as r:
